	...
	if(0 <= xt and xt < wt and 0 <= yt and yt < ht and mask[curt] > 127.0f)
	{
		...
		if (0 <= yb and yb < hb and 0 <= xb and xb < wb) 
		{
			for(int i = 0; i < 3; i++) 
			{
				...
				if (yb > 0)
				{
					if (yt == 0 or mask[curt-wt] < 127.0f)
						Nb = background[(curb-wb)*3+i];
					else 
						Nb = 0;
				...
				{
					Nb = background[curb*3+i];

				}
				// ***Wb
				if ( xb > 0 )
				{
					if (xt == 0 or mask[curt - 1] < 127.0f)
						Wb = background[(curb-1)*3+i];
					else 
						Wb = 0;
...
__global__ void SOR(float w, float *c_b, float *c_bsor, const int wt, const int ht)
{
	const int xt = blockIdx.x * blockDim.x + threadIdx.x;
	const int yt = blockIdx.y * blockDim.y + threadIdx.y;
	const int curt = wt*yt+xt;
	...
	for(int i=0;i<3;i++)
		c_bsor[curt*3+i] = c_bsor[curt*3+i] * w + (1-w) * c_b[curt*3+i];
